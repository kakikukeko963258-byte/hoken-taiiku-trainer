param(
  [string]$RepoName = "hoken-taiiku-trainer",
  [string]$Description = "Hoken-Taiiku test prep app",
  [switch]$Private
)
...
$headers = @{
  ...
  "User-Agent" = "hoken-taiiku-deploy"
}
...
$parents = @()
if ($ref) {
  $baseCommit = Invoke-GitHub -Method "GET" -Path "/repos/$owner/$RepoName/git/commits/$($ref.object.sha)"
...
$commit = Invoke-GitHub -Method "POST" -Path "/repos/$owner/$RepoName/git/commits" -Body @{
  message = "Publish hoken taiiku app"
  tree = $tree.sha
  parents = $parents
}
